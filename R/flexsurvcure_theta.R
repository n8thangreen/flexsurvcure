...
##' Mixture and Non-Mixture Parametric Cure Models with Fixed Cure Fraction
##'
...
##' @examples
##'
##' flexsurvcure_theta(Surv(rectime,censrec)~group, data=bc, dist="weibull", theta = 0.6)
##' curve(pweibull(x, shape=1.7, scale = 998, lower.tail = F)*0.4 + 0.6, from=0, to=4000, ylim = c(0,1))
##'
##' @export
##'
flexsurvcure_theta <- function(formula, data, weights, bhazard, subset, dist,
                               na.action, link = "logistic", mixture = TRUE, theta, ...) {

  call <- match.call()
  indx <- match(c("formula", "data", "weights", "bhazard", "subset", "na.action"),
  ...
  if (class(dist) == "character") {
    dist_list <- flexsurv.dists[[dist]]   # matches parameters to distns
  ...
  if(mixture) {
    dfns_list = list(
      p =    function(q, ...) pmixsurv(pfun, q, theta = theta, ...),
      d =    function(x, ...) dmixsurv(dfun, pfun, x, theta = theta, ...),
      H =    function(x, ...) Hmixsurv(pfun, x, theta = theta, ...),
